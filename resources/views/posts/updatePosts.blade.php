...
<body class="bg-gray-100 font-sans p-6">
    ...
    <!-- Close Icon -->
    <a href="{{ route('user.profile.show', Session::get('loginEmail')) }}"
    ...
    <div class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <form action="{{ route('update.post.post', $post->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" id="price" name="price" value="{{ $post->rate }}"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500" required>
            </div>
            <span class=" text-red-600">
                @error('price')
            ...
            <div class="mb-6">
                ...
                <input type="number" step="1" id="quantity" name="quantity" value="{{ $post->quantity }}"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500" required>
            </div>
            <span class=" text-red-600">
                @error('quantity')
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-indigo-500" required> {{ $post->description }}</textarea>
            </div>
            <span class=" text-red-600">
                @error('description')
                    {{ $message }}
            ...
            <div class="mb-4">
                ...
                <input type="file" id="image" name="image" class="w-full focus:outline-none" accept="image/*">
            </div>
            <span class=" text-red-600">
                @error('image')
                    {{ $message }}
